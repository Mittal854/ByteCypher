...
//         // ... (compress method remains the same) ...
...
//             for (int j = Math.max(0, i - SEARCH_BUFFER_SIZE); j < i; j++) {
//                 int matchLength = 0;

//                 while ((i + matchLength) < data.length && (j + matchLength) < i && data[j + matchLength] == data[i + matchLength]) {
//                     matchLength++;
//                     if (matchLength >= 255) {
//                         break;
//                     }
...
//                 compressedData.write((byte) bestMatchOffset);
//                 compressedData.write((byte) bestMatchLength);
//                 if (i + bestMatchLength < data.length) {
//                     compressedData.write(data[i + bestMatchLength]);
//                     i += bestMatchLength + 1;
//                 } else {
//                     i += bestMatchLength;
//                 }
//             } else {
//                 compressedData.write((byte) 0);
//                 compressedData.write((byte) 1);
//                 compressedData.write(data[i]);
//                 i++;
//             }
//         }
...
//             int offset = Byte.toUnsignedInt(data[i]);
//             int length = Byte.toUnsignedInt(data[i + 1]);
//             byte nextChar = data[i + 2];
...
//                 if (start < 0 || start > output.size()) {
//                     System.err.println("Decompression Error: Invalid offset: " + offset + " at index " + i);
//                     // Skip the invalid sequence and continue
//                     output.write(nextChar);
//                     i += 3;
//                     continue;
//                 }
